...
def delete_selected():
    ...
    selected_item = listbox1.get(selected_index)
    connection = sqlite3.connect("ekzamen_zadanie.db")
    cursor = connection.cursor()
    cursor.execute("DELETE FROM CategoriesCosts WHERE id = ?", (selected_item[0],))
...
def maxVytratyDate():
    connection = sqlite3.connect("ekzamen_zadanie.db")
    cursor = connection.cursor()
    cursor.execute("SELECT price, nameCategory FROM CategoriesCosts WHERE date = '{entryReports.get()}'")

    max_cost = None
    category_of_max_cost = None
    ...
    for row in cursor.fetchall():
        ...
        if max_cost is None or value > max_cost:
            max_cost = value
            category_of_max_cost = category

    if max_cost is not None:
        text1 = f"Max Cost in this day: {max_cost} (Category: {category_of_max_cost})"
    else:
        text1 = "No records found for this date."

    listbox2.insert(END, text1)
...
def minVytratyDate():
    ...
    cursor.execute(f"SELECT price, nameCategory FROM CategoriesCosts WHERE date = '{entryReports.get()}'")

    min_cost = None
    category_of_min_cost = None
    ...
    for row in cursor.fetchall():
        ...
        if min_cost is None or value < min_cost:
            min_cost = value
            category_of_min_cost = category

    if min_cost is not None:
        text1 = f"Min Cost in this day: {min_cost} (Category: {category_of_min_cost})"
    else:
        text1 = "No records found for this date."
...
button3 = Button(text="Delete Category", command=delete_selected)
...
buttonMinDate = Button(text="Reports from Min Date", command=minVytratyDate)
